...
- **Transition** (real first-writer-wins over a non-empty list of `nns-claim-candidate`s) — the part that still panics in the prover.
...
| All list walking, `valid-claim-candidate`, and `name-key` inlined into the formula | Eliminates every `%9` opcode from the trace. | Done (see the big inlined block inside `++recursive-transition-formula`) |
...
| Empty-cands fast path (tiny hand-written 0-6 formula) | Proves "no candidates in this block → acc unchanged" without the full list machinery. Useful for the first real blocks. | Done |
...
- **`++prekey-candidates`** (off-trace, Rust → Hoon conversion):
  ```hoon
  =/  keyed=transition-candidate
    :*  (name-key:na name.c)
        (based name.c)
        (based owner.c)
...
- **`++recursive-transition-formula`** (the giant gate that gets traced):
  - Has an **empty-cands fast path** (tiny 0-6 formula).
  - Has a **non-empty path** that:
    - Receives `packed-prev`, `prev-height`, `acc-list`, `cands`, `pag`.
...
- Many `~&` debug prints were added at the very start of the handler, after building subject/form, at the top of the formula, and at the start of the non-empty `cands` arm. **None of them have ever fired on a non-empty transition** — the crash is before Hoon code runs.
...
- Updated `prekey-candidates` to wrap every field:
  - `(based name.c)`
  - `(based owner.c)`
  - `(based (atom-to-tx-id tx-hash.c))`
...
- When `build_prove_recursive_transition_poke` (or the test helper) builds the `cands` list and the 5-limb `tx-id` cells, it may be producing left-nested cells or flat atoms instead of the exact right-nested cell tree that Hoon `[@ux @ux @ux @ux @ux]` and the `based` map expect.
...
- The empty-cands fast path works (tiny subject).
- Any non-empty `cands` list (even with the "temporary simplification" that removes all predicate checks) produces the crash immediately when `prove-computation:vp` is called.
...
1. **Further simplify the non-empty branch** (even more than the current "TEMPORARY SIMPLIFICATION"):
   ...
   - Make the subject for a non-empty transition as small as possible — just the `acc-list`, the `cands` list (already pre-keyed and based), and the current page.
   - This mirrors exactly what was done for the empty-cands fast path.
...
2. **Audit the Rust noun builder** for the transition poke:
   - Look at how `build_prove_recursive_transition_poke` (and the test helper that creates `ClaimCandidate`s) constructs:
     ...
     - The list of `transition-candidate` (the `cands` field).
...
**Current blocking question**: "Why does the prover reject the subject noun for any non-empty `cands` list before a single Hoon instruction executes?"
